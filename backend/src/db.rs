...
impl DbError {
    /// Build a local (non-driver) error visible to other modules in this crate.
    pub(crate) fn failure(message: impl Into<String>) -> DbError {
        DbError::local(message)
    }
...
impl Row {
    ...
    /// Read a column as `f64`. Integers coerce, matching SQLite's numeric affinity.
    pub fn number(&self, col: &str) -> Option<f64> {
        match self.get(col) {
...
impl Pool {
    ...
    /// panics (the guard's `Drop` does it). `E` is generic so callers can use
    /// [`DbError`] or a domain error that implements `From<DbError>`.
    ///
    /// # Errors
    /// Returns the pool error (closed, missing connection) converted into `E`,
    /// or whatever `f` returns.
    pub fn with<T, E>(&self, f: impl FnOnce(&Db) -> Result<T, E>) -> Result<T, E>
    where
        E: From<DbError>,
    {
        let guard = self.checkout().map_err(E::from)?;
        match guard.db.as_ref() {
            Some(db) => f(db),
            // Unreachable: `checkout` only returns a populated guard.
            None => Err(E::from(DbError::local("pooled connection was missing"))),
        }
    }
...
/// FanFood tables. `CREATE TABLE IF NOT EXISTS` does not widen an older
/// `Orders` table, so [`ORDERS_COLUMNS`] is applied afterwards.
const FANFOOD_SCHEMA: &[&str] = &[
...
/// Columns an older `Orders` table (stadium-only) may be missing.
...
const FANFOOD_INDEXES: &[&str] = &[
    "CREATE INDEX IF NOT EXISTS idx_venue_sections_venue ON VenueSections(venue_id)",
    "CREATE INDEX IF NOT EXISTS idx_menu_items_venue ON MenuItems(venue_id)",
...
/// Create FanFood tables, widen a pre-venue `Orders` table, then seed when empty.
fn ensure_fanfood(db: &Db) -> Result<(), DbError> {
    for statement in FANFOOD_SCHEMA {
        db.exec(statement)?;
    }
    add_missing_columns(db, "Orders", ORDERS_COLUMNS)?;
    for statement in FANFOOD_INDEXES {
        db.exec(statement)?;
    }
    crate::fanfood::seed_if_empty(db)
}
