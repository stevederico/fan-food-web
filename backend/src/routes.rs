...
fn dispatch(state: &AppState, req: &Request) -> Response {
    match (req.method.as_str(), req.path.as_str()) {
        ...
        // Literal collections before `/api/venues/:slug` and `/api/orders/:id`.
        ("GET", "/api/venues") => venues_list(state),
        ("GET", "/api/orders") => orders_list(state, req),
        ("POST", "/api/orders") => orders_create(state, req),
        ("GET", "/api/admin/venues") => admin_venues_list(state, req),
        ("POST", "/api/admin/venues") => admin_venue_create(state, req),
        (_m, p) if p.starts_with("/api/") => dispatch_fanfood(state, req).unwrap_or_else(not_found),
...
fn signup(state: &AppState, req: &Request) -> Response {
    ...
    let body = json::obj([
        ...
        ("email", json::s(email.clone())),
        ("name", json::s(name.trim())),
        ("isAdmin", Json::Bool(crate::fanfood::is_admin_email(&email))),
...
fn signin(state: &AppState, req: &Request) -> Response {
    ...
    m.insert(
        "isAdmin".into(),
        Json::Bool(crate::fanfood::is_admin_email(&user.email)),
    );
...
/// Parameterized FanFood paths. Literal collections are matched in [`dispatch`] first.
fn dispatch_fanfood(state: &AppState, req: &Request) -> Option<Response> {
    let parts: Vec<&str> = req.path.split('/').filter(|s| !s.is_empty()).collect();
    let method = req.method.as_str();
    match (method, parts.as_slice()) {
        ("GET", ["api", "venues", slug, "menu"]) => Some(venue_menu(state, slug)),
        ("GET", ["api", "venues", slug, "sections"]) => Some(venue_sections(state, slug)),
        ("GET", ["api", "venues", slug]) => Some(venue_detail(state, slug)),
        ("GET", ["api", "orders", id]) => Some(order_detail(state, req, id)),
        ("PUT", ["api", "admin", "venues", id]) => Some(admin_venue_update(state, req, id)),
        ("GET", ["api", "admin", "venues", id, "menu"]) => Some(admin_menu_list(state, req, id)),
        ("POST", ["api", "admin", "venues", id, "menu"]) => Some(admin_menu_create(state, req, id)),
        ("POST", ["api", "admin", "venues", id, "sections"]) => Some(admin_section_create(state, req, id)),
        ("PUT", ["api", "admin", "menu", id]) => Some(admin_menu_update(state, req, id)),
        ("PUT", ["api", "admin", "sections", id]) => Some(admin_section_update(state, req, id)),
        _ => None,
    }
}
...
fn finish<T>(
    ...
    context: &str,
    server_msg: &str,
    result: Result<T, crate::fanfood::FanFail>,
    ok: impl FnOnce(T) -> Response,
) -> Response {
    match result {
        Ok(value) => ok(value),
        Err(crate::fanfood::FanFail::Http(status, msg)) => err_json(status, &msg),
        Err(crate::fanfood::FanFail::Db(e)) => {
            state
                .log
                .error(context, &[("error", json::s(e.to_string()))]);
            err_json(500, server_msg)
        }
    }
}

fn json_opt_str(value: Option<&str>) -> Json {
    match value {
        Some(s) => json::s(s),
        None => Json::Null,
    }
...
fn venue_json(v: &crate::fanfood::Venue) -> Json {
    json::obj([
        ("active", Json::Bool(v.active)),
        ("address", json::s(v.address.as_str())),
        (
            "capacity",
            v.capacity.map(json::i).unwrap_or(Json::Null),
        ),
        ("city", json::s(v.city.as_str())),
        ("deliveryMode", json::s(v.delivery_mode.as_str())),
        ("id", json::s(v.id.as_str())),
        ("name", json::s(v.name.as_str())),
        ("shortName", json::s(v.short_name.as_str())),
        ("slug", json::s(v.slug.as_str())),
        ("state", json::s(v.state.as_str())),
        ("timezone", json::s(v.timezone.as_str())),
    ])
}

fn section_json(s: &crate::fanfood::VenueSection) -> Json {
    json::obj([
        ("code", json::s(s.code.as_str())),
        ("deliveryEligible", Json::Bool(s.delivery_eligible)),
        ("id", json::s(s.id.as_str())),
        ("level", json::s(s.level.as_str())),
        ("notes", json_opt_str(s.notes.as_deref())),
        ("rowMax", json_opt_str(s.row_max.as_deref())),
        ("rowMin", json_opt_str(s.row_min.as_deref())),
        ("sortOrder", json::i(s.sort_order)),
        ("venueId", json::s(s.venue_id.as_str())),
        ("zone", json::s(s.zone.as_str())),
    ])
}

fn menu_json(item: &crate::fanfood::MenuItem) -> Json {
    json::obj([
        ("active", Json::Bool(item.active)),
        ("category", json::s(item.category.as_str())),
        ("description", json_opt_str(item.description.as_deref())),
        ("id", json::s(item.id.as_str())),
        ("name", json::s(item.name.as_str())),
        ("price", json::n(item.price)),
        ("sortOrder", json::i(item.sort_order)),
        ("venueId", json::s(item.venue_id.as_str())),
    ])
}

fn order_json(order: &crate::fanfood::FoodOrder) -> Json {
    json::obj([
        ("confirmNumber", json::s(order.confirm_number.as_str())),
        ("createdAt", json::i(order.created_at)),
        ("deliveryEligible", Json::Bool(order.delivery_eligible)),
        ("foodType", json::s(order.food_type.as_str())),
        ("id", json::s(order.id.as_str())),
        ("level", json_opt_str(order.level.as_deref())),
        ("menuItemId", json_opt_str(order.menu_item_id.as_deref())),
        ("paymentType", json::s(order.payment_type.as_str())),
        ("qty", json::i(order.qty)),
        ("row", json::s(order.row.as_str())),
        ("seat", json::s(order.seat.as_str())),
        ("section", json::s(order.section.as_str())),
        ("status", json::s(order.status.as_str())),
        ("totalPrice", json::n(order.total_price)),
        ("userId", json::s(order.user_id.as_str())),
        ("venueId", json::s(order.venue_id.as_str())),
        ("venueName", json::s(order.venue_name.as_str())),
        ("zone", json_opt_str(order.zone.as_deref())),
    ])
}

fn json_list(items: impl IntoIterator<Item = Json>) -> Json {
...
fn venue_detail(state: &AppState, slug: &str) -> Response {
    ...
    finish(
        ...
        state
            .pool
            .with(move |db| crate::fanfood::find_active_venue(db, &slug).map_err(crate::fanfood::FanFail::from))
            .and_then(|found| found.ok_or_else(|| crate::fanfood::FanFail::Http(404, "Venue not found".into()))),
        |venue| json_res(200, &venue_json(&venue)),
    )
}

fn venue_menu(state: &AppState, slug: &str) -> Response {
    let slug = slug.to_string();
    finish(
        state,
        "Get menu error",
        "Failed to load menu",
        state.pool.with(move |db| crate::fanfood::active_menu(db, &slug)),
        |(venue, items)| {
            json_res(200, &json_list(items.iter().map(menu_json))).header("X-Venue", &venue.slug)
        },
    )
}

fn venue_sections(state: &AppState, slug: &str) -> Response {
    let slug = slug.to_string();
    finish(
        state,
        "Get sections error",
        ...
        state
            .pool
            .with(move |db| crate::fanfood::active_sections(db, &slug)),
        |rows| json_res(200, &json_list(rows.iter().map(section_json))),
    )
}

fn orders_list(state: &AppState, req: &Request) -> Response {
    let user_id = match require_auth(state, req) {
        Ok(id) => id,
        Err(r) => return r,
    };
    match state
        .pool
        .with(|db| crate::fanfood::list_orders(db, &user_id))
    {
        Ok(rows) => json_res(200, &json_list(rows.iter().map(order_json))),
        Err(e) => {
            state
                .log
                .error("List orders error", &[("error", json::s(e.to_string()))]);
            err_json(500, "Failed to load orders")
        }
    }
}

fn order_detail(state: &AppState, req: &Request, id: &str) -> Response {
    let user_id = match require_auth(state, req) {
        Ok(id) => id,
        Err(r) => return r,
    };
    if id.is_empty() {
        return err_json(400, "Order id required");
    }
    let id = id.to_string();
    match state
        .pool
        .with(|db| crate::fanfood::find_order(db, &id, &user_id))
    {
        Ok(Some(order)) => json_res(200, &order_json(&order)),
        Ok(None) => err_json(404, "Order not found"),
        Err(e) => {
            state
                .log
                .error("Get order error", &[("error", json::s(e.to_string()))]);
            err_json(500, "Failed to load order")
        }
    }
}

fn orders_create(state: &AppState, req: &Request) -> Response {
    let user_id = match require_auth(state, req) {
        Ok(id) => id,
        Err(r) => return r,
    };
    ...
    let venue_key = body
        ...
        .or_else(|| body.get_str("venueId"))
        .unwrap_or("")
        .trim()
        .to_string();
    let menu_item_id = body.get_str("menuItemId").unwrap_or("").trim().to_string();
    let section_code = body.get_str("section").unwrap_or("").trim().to_string();
    let row = validation::escape_html(body.get_str("row").unwrap_or("").trim());
    let seat = validation::escape_html(body.get_str("seat").unwrap_or("").trim());
    let payment = if body.get_str("paymentType") == Some("Card") {
        "Card"
    } else {
        "Cash"
    };
    let Some(qty) = crate::fanfood::parse_qty(body.get("qty")) else {
        return err_json(400, "Quantity must be between 1 and 20");
    };
    let input = crate::fanfood::PlaceOrder {
    ...
    let result = state.pool.with(|db| crate::fanfood::place_order(db, &input));
    if let Ok(order) = &result {
        state.log.info(
            "Order placed",
            &[
                ("orderId", json::s(order.id.as_str())),
                ("userID", json::s(order.user_id.as_str())),
                ("food", json::s(order.food_type.as_str())),
                ("section", json::s(order.section.as_str())),
            ],
        );
    }
    finish(state, "Create order error", "Failed to place order", result, |order| {
        json_res(201, &order_json(&order))
    })
}

/// Auth plus admin gate. CSRF runs first on mutations, matching the Node middleware order.
fn require_admin(state: &AppState, req: &Request, csrf: bool) -> Result<(), Response> {
    let user_id = require_auth(state, req)?;
    if csrf {
        require_csrf(state, req, &user_id)?;
    }
    let user = match state.pool.find_user(&UserQuery::Id(user_id)) {
        Ok(Some(user)) => user,
        Ok(None) => return Err(err_json(403, "Admin access required")),
        Err(e) => return Err(db_err(state, "Admin auth error", &e)),
    };
    if crate::fanfood::is_admin_email(&user.email) {
        Ok(())
    } else {
        Err(err_json(403, "Admin access required"))
    }
}

fn admin_venues_list(state: &AppState, req: &Request) -> Response {
    if let Err(res) = require_admin(state, req, false) {
        return res;
    }
...
fn admin_venue_create(state: &AppState, req: &Request) -> Response {
    if let Err(res) = require_admin(state, req, true) {
        return res;
    }
    let body = match parse_json_body(req) {
        Ok(v) => v,
    ...
    let result = state.pool.with(|db| crate::fanfood::create_venue(db, &body));
    if let Ok(venue) = &result {
        state.log.info(
            "Admin created venue",
            &[
                ("venueId", json::s(venue.id.as_str())),
                ("slug", json::s(venue.slug.as_str())),
            ],
        );
    }
    finish(
        state,
        "Admin create venue error",
        "Failed to create venue",
        result,
        |venue| json_res(201, &venue_json(&venue)),
    )
}

fn admin_venue_update(state: &AppState, req: &Request, id: &str) -> Response {
    if id.is_empty() {
        return err_json(400, "Venue id required");
    }
    if let Err(res) = require_admin(state, req, true) {
        return res;
    }
    ...
    finish(
        state,
        "Admin update venue error",
        "Failed to update venue",
        state
            .pool
            .with(move |db| crate::fanfood::update_venue(db, &id, &body)),
        |venue| json_res(200, &venue_json(&venue)),
    )
}

fn admin_menu_list(state: &AppState, req: &Request, id: &str) -> Response {
    if id.is_empty() {
        return err_json(400, "Venue id required");
    }
    if let Err(res) = require_admin(state, req, false) {
...
fn admin_menu_create(state: &AppState, req: &Request, id: &str) -> Response {
    if id.is_empty() {
        return err_json(400, "Venue id required");
    }
    if let Err(res) = require_admin(state, req, true) {
    ...
    finish(
        state,
        "Admin add menu error",
        "Failed to add menu item",
        state
            .pool
            .with(move |db| crate::fanfood::create_menu_item(db, &id, &body)),
        |item| json_res(201, &menu_json(&item)),
    )
}

fn admin_menu_update(state: &AppState, req: &Request, id: &str) -> Response {
    if id.is_empty() {
        return err_json(400, "Menu item id required");
    }
    if let Err(res) = require_admin(state, req, true) {
        return res;
    }
    ...
    finish(
        state,
        "Admin update menu error",
        "Server error",
        state
            .pool
            .with(move |db| crate::fanfood::update_menu_item(db, &id, &body)),
        |item| json_res(200, &menu_json(&item)),
    )
}

fn admin_section_create(state: &AppState, req: &Request, id: &str) -> Response {
    if id.is_empty() {
        return err_json(400, "Venue id required");
    }
    if let Err(res) = require_admin(state, req, true) {
    ...
    finish(
        state,
        "Admin add section error",
        "Failed to add section",
        state
            .pool
            .with(move |db| crate::fanfood::create_section(db, &id, &body)),
        |section| json_res(201, &section_json(&section)),
    )
}

fn admin_section_update(state: &AppState, req: &Request, id: &str) -> Response {
    if id.is_empty() {
        return err_json(400, "Section id required");
    }
    if let Err(res) = require_admin(state, req, true) {
        return res;
    }
    ...
    finish(
        state,
        "Admin update section error",
        "Server error",
        state
            .pool
            .with(move |db| crate::fanfood::update_section(db, &id, &body)),
        |section| json_res(200, &section_json(&section)),
    )
}
...
mod tests {
    ...
    fn menu_item<'a>(body: &'a Json, name: &str) -> &'a Json {
        body.as_arr()
            .unwrap()
            .iter()
            .find(|item| item.get_str("name") == Some(name))
            .unwrap_or_else(|| panic!("missing menu item {name}"))
    }

    #[test]
    fn venues_list_includes_seeded_oracle_park() {
        let (state, dir) = test_state();
        let res = handle(&state, Request::for_test("GET", "/api/venues"));
        assert_eq!(res.status, 200, "{}", String::from_utf8_lossy(&res.body));
        let body = json_body(&res);
        let venue = body
            .as_arr()
            ...
            .expect("oracle park");
        assert_eq!(venue.get_str("deliveryMode"), Some("premium"));
        assert_eq!(venue.get("active").and_then(Json::as_bool), Some(true));
        std::fs::remove_dir_all(&dir).ok();
    }

    #[test]
    fn menu_path_is_not_a_venue_slug() {
        let (state, dir) = test_state();
        let res = handle(
            &state,
        ...
        assert_eq!(res.status, 200, "{}", String::from_utf8_lossy(&res.body));
        assert_eq!(res_header(&res, "X-Venue"), Some("oracle-park"));
        let menu_body = json_body(&res);
        let fries = menu_item(&menu_body, "Garlic Fries");
        let price = fries.get("price").and_then(Json::as_f64).unwrap();
        assert!((price - 12.5).abs() < 0.001, "{price}");
        let detail = handle(
            &state,
            Request::for_test("GET", "/api/venues/oracle-park"),
        );
        assert_eq!(detail.status, 200);
        assert_eq!(json_body(&detail).get_str("slug"), Some("oracle-park"));
        std::fs::remove_dir_all(&dir).ok();
    }

    #[test]
    fn field_club_section_is_delivery_eligible() {
        let (state, dir) = test_state();
        let res = handle(
            &state,
            Request::for_test("GET", "/api/venues/oracle-park/sections"),
        );
        assert_eq!(res.status, 200, "{}", String::from_utf8_lossy(&res.body));
        let rows = json_body(&res);
        let club = rows
            .as_arr()
            .unwrap()
            .iter()
            .find(|s| s.get_str("code") == Some("122"))
            .unwrap();
        let boxes = rows
            .as_arr()
            .unwrap()
            .iter()
            .find(|s| s.get_str("code") == Some("101"))
            .unwrap();
        assert_eq!(club.get("deliveryEligible").and_then(Json::as_bool), Some(true));
        assert_eq!(boxes.get("deliveryEligible").and_then(Json::as_bool), Some(false));
        std::fs::remove_dir_all(&dir).ok();
    }

    #[test]
    fn order_price_comes_from_the_menu_and_stays_with_the_owner() {
        let (state, dir) = test_state();
        let menu = handle(
            &state,
            Request::for_test("GET", "/api/venues/oracle-park/menu"),
        ...
        let ada = signed_up(&state, "ada-order@example.com");
        let mut req = Request::for_test("POST", "/api/orders");
        replay_cookies(&mut req, &[&ada], true);
        req.set_test_body(
            format!(
                r#"{{"venueSlug":"oracle-park","menuItemId":"{item_id}","section":"122","row":"A","seat":"1","qty":2,"price":0.01,"paymentType":"Cash"}}"#
            )
            .into_bytes(),
        );
        ...
        let total = order.get("totalPrice").and_then(Json::as_f64).unwrap();
        assert!((total - 25.0).abs() < 0.001, "{total}");
        assert_eq!(order.get_str("status"), Some("Ordered"));
        assert_eq!(order.get_str("foodType"), Some("Garlic Fries"));
        let order_id = order.get_str("id").unwrap();

        let mut mine = Request::for_test("GET", &format!("/api/orders/{order_id}"));
        replay_cookies(&mut mine, &[&ada], false);
        let mine_res = handle(&state, mine);
        assert_eq!(mine_res.status, 200, "{}", String::from_utf8_lossy(&mine_res.body));

        let bob = signed_up(&state, "bob-order@example.com");
        let mut theirs = Request::for_test("GET", &format!("/api/orders/{order_id}"));
        replay_cookies(&mut theirs, &[&bob], false);
        let theirs_res = handle(&state, theirs);
        assert_eq!(theirs_res.status, 404);

        let anon = handle(&state, Request::for_test("POST", "/api/orders"));
        assert_eq!(anon.status, 401);
        std::fs::remove_dir_all(&dir).ok();
    }

    #[test]
    fn admin_venues_literal_path_requires_admin() {
        let _guard = ENV_LOCK.lock().unwrap_or_else(|e| e.into_inner());
        let prev = std::env::var("ADMIN_EMAILS").ok();
        // SAFETY: ENV_LOCK is held for the whole test. Restored before return.
        unsafe { std::env::set_var("ADMIN_EMAILS", "owner@example.com") };
        let (state, dir) = test_state_locked();
        let fan = signed_up(&state, "fan-admin@example.com");
        let mut req = Request::for_test("GET", "/api/admin/venues");
        replay_cookies(&mut req, &[&fan], false);
        let denied = handle(&state, req);
        assert_eq!(denied.status, 403, "{}", String::from_utf8_lossy(&denied.body));

        unsafe { std::env::set_var("ADMIN_EMAILS", "fan-admin@example.com") };
        let mut allowed = Request::for_test("GET", "/api/admin/venues");
        replay_cookies(&mut allowed, &[&fan], false);
        let ok = handle(&state, allowed);
        assert_eq!(ok.status, 200, "{}", String::from_utf8_lossy(&ok.body));
        assert!(json_body(&ok).as_arr().unwrap().len() >= 1);

        let mut create = Request::for_test("POST", "/api/admin/venues");
        replay_cookies(&mut create, &[&fan], true);
        create.set_test_body(
            br#"{"name":"Test Park","city":"Oakland","state":"CA","address":"1 Test Way","deliveryMode":"all"}"#
                .to_vec(),
        );
