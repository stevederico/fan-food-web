...
- App `version` tracks `CHANGELOG.md`. `skateboardVersion` tracks the boilerplate pin. Do not leave `skateboardVersion` stale after an updater run.
...
### Domain rules
- Venues, sections, and menu items live in **SQLite** (`backend/src/db.rs`) — never hardcode a single stadium for new features
...
### Dev notes
- Prefer `bun run front` + `cd backend && cargo run`
...
│   └── constants.json
├── backend/
│   ├── src/              # Auth + FanFood routes (zero-crate Rust)
